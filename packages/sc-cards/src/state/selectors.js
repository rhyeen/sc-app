...
function _getHandCard(handCard, cards) {
  return {
    ...handCard,
    card: CardBuilder.buildCard(cards, handCard.id, handCard.instance),
  };
}
...
export const getSelectedCard = createSelector(
  ...
  (selectedCard, cards) => ({
    ...selectedCard,
    card: CardBuilder.buildCard(cards, selectedCard.id, selectedCard.instance),
  }),
);
...
export const getSelectedAbility = createSelector(
  ...
  (selectedCard, selectedAbility, cards) => {
    const card = CardBuilder.buildCard(cards, selectedCard.id, selectedCard.instance);
    return {
      ...selectedCard,
      ...selectedAbility,
      card,
...
function _getFieldSlot(fieldSlots, playAreaIndex, cards) {
  return {
    ...
    card: CardBuilder.buildCard(
      cards,
      fieldSlots[playAreaIndex].id,
      fieldSlots[playAreaIndex].instance,
    ),
  };
}
...
export const getOpponentFieldBacklogSizes = createSelector(
  ...
  backlog => [backlog[0].size, backlog[1].size, backlog[2].size],
);
