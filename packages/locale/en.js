...
export const LOCALE_EN = {
  ...
  SC_ROOT: {
    PAGE_NOT_FOUND: {
      ...
      P2: html`
        Let's <a href="/">get back</a> to the fun.
      `,
    },
    TITLE: {
  ...
  SC_CRAFT: {
    CRAFTING_PARTS: {
      FORGE_NOT_EMPTY: amount =>
        html`
          Add <span class="dynamic-value">${amount}</span> to a card in the forge.
        `,
      FORGE_EMPTY: 'Unlock by adding a card to the forge.',
      NO_USES_REMAIN: 'No uses left this turn',
    },
    CRAFTING_CARD_SLOTS: {
      ABILITY_COUNT: amount =>
        amount === 1
          ? html`
              <span class="count">1</span> ability
            `
          : html`
              <span class="count">${amount}</span> abilities
            `,
      SLOT_COUNT: amount =>
        amount === 1
          ? html`
              <span class="count">1</span> slot left
            `
          : html`
              <span class="count">${amount}</span> slots left
            `,
    },
  ...
  ABILITY: {
    ENERGIZE: {
      ...
      MODIFIED_DESCRIPTION: (amount, modifier) =>
        html`
          +${amount} <span class="modifier">(${modifier})</span> max and current energy.
        `,
    },
    HASTE: {
      NAME: 'haste',
    ...
    SPELLSHOT: {
      ...
      MODIFIED_DESCRIPTION: (amount, modifier) =>
        html`
          Deal ${amount} <span class="modifier">(${modifier})</span> to enemy unit.
        `,
    },
    REACH: {
      NAME: 'reach',
      ...
      MODIFIED_DESCRIPTION: (amount, modifier) =>
        html`
          +${amount} <span class="modifier">(${modifier})</span> range
        `,
    },
  },
  CARD_STAT: {
